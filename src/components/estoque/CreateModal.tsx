...
export default function CreateModal(props:ICreateModal) {
    ...
    function addNewAdditing(){
        ...
        newList.innerHTML = "<input type='text' placeholder='Nome' class='bg-zinc-700 py-2 px-4 additingName w-2/5'/><input type='number' placeholder='Preço' class='w-2/5 bg-zinc-700 py-2 px-4 additingPrice'/><span class='ml-5 bg-red-700 px-4 py-1 rounded-2xl'>Remover</span>"
        newList.childNodes[2].addEventListener("click", ()=>{
    ...
    useEffect(()=>{
        if(props.show){
            if(modalRef.current?.classList.contains("hidden")) {
                ...
                IngredientsRef.current!.innerHTML = ""
            }
        }else{
